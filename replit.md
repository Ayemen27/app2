...
- **Offline System Architecture:** World-class offline-first PWA with: (1) `store-registry.ts` — single canonical source of truth for ALL endpoint↔store↔queryKey↔serverTable mappings used by queryClient, interceptor, sync, and offline-queries; (2) Service Worker (`sw.js`) with cache-first for static assets, network-first for API, navigation fallback to `/offline.html`; (3) `sync-leader.ts` — BroadcastChannel-based multi-tab leader election ensuring only one tab processes sync queue; (4) `storage-recovery.ts` — staged QuotaExceeded cleanup and IDB corruption detection/recovery; (5) IndexedDB v15 with 52+ stores including `equipment` and `equipmentMovements`; (6) Auto-caching of API responses to IndexedDB with `value`-based ID normalization for non-id datasets (worker-types).
