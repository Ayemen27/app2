...
export async function exportActivitiesToExcel(
  ...
  activities.forEach((activity, idx) => {
    ...
    const dateTime = createdAt ? new Date(createdAt) : new Date();
    const isValidDate = !isNaN(dateTime.getTime());
    
    const formattedDate = isValidDate ? dateTime.toLocaleDateString('ar-SA') : '-';
    const formattedTime = isValidDate ? dateTime.toLocaleTimeString('ar-SA', { 
      hour: '2-digit', 
      minute: '2-digit',
      hour12: true 
    }) : '-';
