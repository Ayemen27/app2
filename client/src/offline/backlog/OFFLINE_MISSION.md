...
### 1. إعادة هيكلة طبقة البيانات (Local-First Access)
...
- **معايير القبول:**
    - نجاح عملية الحفظ محلياً بنسبة 100% حتى في حالة `status: 502` للسيرفر.
    - عدم وجود أي `await` لطلبات الشبكة في المسار الحرج للمستخدم.

### 2. تطوير محرك المزامنة الخلفي (Background Sync Engine)
...
- **معايير القبول:**
    - المزامنة تتم في صمت (Silent Sync).
    - التعامل مع تعارض البيانات (Conflict Resolution) بناءً على "الأحدث محلياً هو الأصح".

### 3. واجهة المستخدم (Optimistic UI & Indicators)
- **المهمة:** تعديل المكونات لاستخدام `Optimistic Updates` من React Query بشكل كامل.
- **معايير القبول:**
    - استجابة فورية للواجهة (Immediate Feedback).
