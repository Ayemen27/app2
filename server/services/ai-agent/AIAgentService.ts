...
تذكر: أمر ACTION واحد فقط. لا تذكر ACTION للمستخدم. لا تختلق بيانات.`;
  ...
  private detectIntentFromUserMessage(message: string): string | null {
    ...
    const patterns: [RegExp, string][] = [
      ...
      [/(?:تحليل\s*(?:ال)?آبار|تحليل\s*شامل\s*(?:ال)?آبار|إحصائيات\s*(?:ال)?آبار|well\s*analysis)/i, "[ACTION:WELL_ANALYSIS]"],
      [/(?:ملخص\s*(?:ال)?فرق|ملخص\s*فرق\s*العمل|جميع\s*(?:ال)?فرق|كل\s*(?:ال)?فرق|crew\s*summary)/i, "[ACTION:WELL_CREWS_SUMMARY]"],
      [/(?:مقارن[ةه]\s*(?:ال)?آبار|مقارن[ةه]\s*مشاريع\s*(?:ال)?آبار|well\s*comparison)/i, "[ACTION:WELL_COMPARISON]"],
    ...
    const wellSearchMatch = msg.match(/(?:ابحث|بحث)\s*(?:في|عن)?\s*(?:ال)?آبار\s+(.+)/i);
