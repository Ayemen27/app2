...
- **Financial Architecture Audit & Fixes (SSOT Enforcement):** Comprehensive audit identified and fixed 4 critical bugs:
  1. **FinancialLedgerService.invalidateSummaries** was using Drizzle `db.insert(summaryInvalidations)` with different column schema — now delegates to `SummaryRebuildService.markInvalid()` (the canonical invalidation path with UPSERT/LEAST logic).
