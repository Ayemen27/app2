...
- **Financial Integrity System (March 2026):** Comprehensive overhaul to match global accounting standards:
  ...
  - **NUM() Helper Unification (March 2026):** Replaced all `CAST(COALESCE(NULLIF(field::text,''),'0') AS DECIMAL)` patterns (35+ occurrences) across `ReportDataService.ts` with: `NUM(field)` for Drizzle sql templates (using `safe_numeric(col::text, 0)` helper), and `safe_numeric(field::text)` for raw SQL queries. Zero legacy CAST patterns remain in the file.
