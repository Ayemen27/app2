...
- **Professional Reports:** Generation of detailed financial and operational reports in Excel and PDF formats, with customizable date ranges and project filtering. PDF export opens styled HTML with a print bar for browser-native PDF save. Period Final report includes project fund transfers (ترحيل الأموال بين المشاريع) with incoming/outgoing tracking. Worker Statement shows cross-project data by default with professional project summary (per-project days/earnings/balance + totals). Worker balance: `balance = totalEarned - (totalDirectPaid + totalTransfers)`. Project balance: `(totalFundTransfers + totalProjectTransfersIn) - totalExpenses`. Multi-Project Combined Report (تقرير مجمع) allows selecting multiple projects with checkbox UI, shows per-project breakdown cards + combined totals, inter-project transfers between selected projects (excluded from combinedExpenses to prevent double-counting), merged worker/material/fund-transfer tables with project name column, area/pie charts, and Excel/PDF export via `MultiProjectFinalExcel.ts`/`MultiProjectFinalPDF.ts`. API: `GET /api/reports/v2/multi-project-final?project_ids=id1,id2&dateFrom&dateTo`.
