...
- **WhatsApp Chat Export → Accounting Pipeline (March 2026):** Multi-phase pipeline converting WhatsApp conversations into structured financial entries with mandatory human approval.
  ...
  - **Task #4 (Review Dashboard & Posting Engine):** WhatsAppPostingService.ts (atomic posting via pool/client, dual idempotency, re-approval guard), 7 API routes (approve/reject/bulk-approve/dry-run/post/custodian-statements/review-actions), wa-import.tsx React dashboard (6 tabs: batches, review, reconciliation, custodians, aliases, loans). Route at `/wa-import` with editor-level access. Posting admin-only. **Floating upload button (FAB)** for ZIP import with progress indicator.
  - **P0 Hardening (March 2026):**
    - **Unified numeric safety:** All `parseFloat` calls across 8 WA service files replaced with `safeParseNum` from `server/utils/safe-numbers.ts`. NaN never propagates to DB amounts.
    - **DB Transaction wrapping:** `approveCandidate`, `rejectCandidate`, `mergeCandidates`, `splitCandidate` in WhatsAppPostingService.ts + full reconciliation batch in ReconciliationService.ts wrapped in `db.transaction()` for atomic consistency.
    - **Media lifecycle fix:** Removed `rmSync` cleanup in WhatsAppIngestionService.ts — media files now persist after ZIP processing for future OCR support. `ocr_text` column already in schema.
    - **Dynamic projects:** Removed hard-coded `PROJECT_MAP` from frontend. Added `GET /api/wa-import/projects` endpoint with project-scoped access control. Projects loaded dynamically in approve/merge/split dialogs.
    - **Project-scoped authorization (RBAC):** All mutation routes (approve, bulk-approve, merge, split) verify `user_project_permissions` server-side before executing. Non-admin users cannot operate on unauthorized projects even via direct API calls.
    - **Idempotent reconciliation:** `runReconciliation` now skips candidates that already have a `canonicalTransactionId`, preventing duplicate canonical records on re-run.
    - **Schema fix:** Added `reviewNotes` column to `waCanonicalTransactions` (was missing but used by PostingService approve logic).
    - **ArabicAmountParser consistency:** All 9 `parseFloat` calls replaced with `safeParseNum` for codebase uniformity.
    - **Frontend resilience:** All 6 `queryFn` fetches in wa-import.tsx now check `res.ok` before parsing JSON.
    - **Database indexes:** Added indexes on `wa_extraction_candidates.canonical_transaction_id`, `wa_verification_queue.candidate_id` for query performance.
