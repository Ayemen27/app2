...
- **Financial Integrity System (March 2026):** Comprehensive overhaul to match global accounting standards:
  ...
  - **Foreign Key Constraints (March 2026):** Added 13 DB-level FK constraints to 7 previously unprotected tables: worker_transfers (worker_id→workers, project_id→projects), project_fund_transfers (from/to→projects), transportation_expenses (project_id→projects, worker_id→workers), worker_misc_expenses (project_id→projects), supplier_payments (supplier_id→suppliers, project_id→projects, purchase_id→material_purchases), worker_balances (worker_id→workers, project_id→projects), notification_read_states (user_id→users). Added 4 new supporting indexes.
