...
- **WhatsApp Integration:** Utilizes the Baileys library for multi-user WhatsApp sessions with full security isolation. Features `WhatsAppSecurityContext` for per-message user scoping, `ScopedDatabaseActions` to enforce project-level data access, fail-closed whitelist (empty = reject all), and `whatsapp_security_events` audit logging. Admin-only AI commands (SQL_SELECT, LIST_TABLES, ALL_PROJECTS_REPORT) are blocked for regular users.
