    ...
    try {
      ...
      const [prevIncome, prevExpenses] = await Promise.all([
        ...
        db.execute(sql`
          SELECT COALESCE(SUM(CAST(amount AS DECIMAL)), 0) as total
          FROM (
            SELECT total_amount as amount FROM material_purchases WHERE project_id = ${projectId} AND purchase_type = 'نقداً' AND purchase_date::date < ${startDateStr}::date
            UNION ALL
      ...
      const [
        ...
        db.execute(sql`SELECT COUNT(*) as count, COALESCE(SUM(CAST(total_amount AS DECIMAL)), 0) as total FROM material_purchases WHERE project_id = ${projectId} AND purchase_type = 'نقداً' ${dateFilterMp}`),
        db.execute(sql`SELECT COUNT(*) as count, COALESCE(SUM(CAST(total_amount AS DECIMAL)), 0) as total FROM material_purchases WHERE project_id = ${projectId} AND purchase_type = 'آجل' ${dateFilterMp}`),
