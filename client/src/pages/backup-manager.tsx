...
interface BackupLog {
  ...
  errorMessage?: string;
  triggeredBy?: string;
  createdAt?: string; // جعلها اختيارية لمنع الخطأ عند غيابها
}
...
export default function BackupManager() {
  ...
  const { data: rawLogsData, isLoading, refetch } = useQuery<any>({
    queryKey: ["/api/backups/logs"],
    refetchInterval: 5000,
  });

  const logs = useMemo(() => {
    // معالجة البيانات القادمة من API للتأكد من أنها مصفوفة
    if (rawLogsData?.success && Array.isArray(rawLogsData.data)) {
      return rawLogsData.data;
    }
  ...
  const filteredLogs = useMemo(() => {
    return (logs as BackupLog[]).filter(log => {
  ...
  const lastBackup = (logs as BackupLog[]).find(l => l.status === 'success');
      ...
      <div className="mt-6 px-4">
        {isLoading && logs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3">
            {(filteredLogs as BackupLog[]).map((log) => (
              <div 
                key={log.id} 
