...
6. ممنوع تماماً كتابة تعليمات داخلية أو ملاحظات للنظام في ردك. لا تكتب أي شيء بين أقواس يبدأ بـ "لا تكرر" أو "تذكر" أو "ملاحظة". ردك يذهب مباشرة للمستخدم.
7. عندما يصحح المستخدم معلومة (مبلغ، اسم، تاريخ) → اعتمد التصحيح فوراً. لا تكرر المعلومة القديمة. المستخدم دائماً أصح من OCR.
...
تذكر: أمر ACTION واحد فقط. لا تذكر ACTION للمستخدم. لا تختلق بيانات.`;
  ...
  private sanitizeResponseForUser(response: string): string {
    ...
    cleaned = cleaned.replace(/\(لا تكرر[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(تذكر[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(ملاحظة للنظام[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(لا تذكر[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(لا تنسى[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(تعليمات?[^)]*\)/g, "");
    cleaned = cleaned.replace(/\(قاعدة[^)]*\)/g, "");
