...
import type {
  ...
  ProjectComprehensiveReportData,
...
function MultiProjectFinalTab({ onStatsReady }: { onStatsReady?: (stats: any[]) => void }) {
  const { toast } = useToast();
  const [searchValue, setSearchValue] = useState("");
  ...
  const { data: multiReport, isLoading, isError, error, refetch } = useQuery<MultiProjectFinalReportData | null>({
    ...
    queryFn: async () => {
      ...
      const params = new URLSearchParams({
        project_ids: selectedProjectIds.join(","),
        dateFrom,
        dateTo,
      });
  ...
  const handleExport = (fmt: "xlsx" | "pdf") => {
    ...
    secureDownloadExport("multi-project-final", fmt, {
      project_ids: selectedProjectIds.join(","),
      dateFrom,
      dateTo,
    }, toast);
            ...
            {allProjects.map((p: any) => {
              ...
              return (
                <button
                  ...
                  data-testid={`btn-project-toggle-${p.id}`}
        ...
        return (
          <>
              ...
              <Card key={proj.projectId}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    ملخص: {proj.projectName}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="text-center p-3 rounded-md bg-blue-50 dark:bg-blue-950/30" data-testid={`stat-income-${proj.projectId}`}>
                      <p className="text-xs text-muted-foreground">الوارد</p>
                      <p className="font-bold text-sm mt-1 text-blue-600 dark:text-blue-400">{formatCurrency(proj.totals.totalIncome)}</p>
                    </div>
                    <div className="text-center p-3 rounded-md bg-red-50 dark:bg-red-950/30" data-testid={`stat-expenses-${proj.projectId}`}>
                      <p className="text-xs text-muted-foreground">المصروفات</p>
                      <p className="font-bold text-sm mt-1 text-red-600 dark:text-red-400">{formatCurrency(proj.totals.totalExpenses)}</p>
                    </div>
                    <div className="text-center p-3 rounded-md bg-green-50 dark:bg-green-950/30" data-testid={`stat-balance-${proj.projectId}`}>
                      <p className="text-xs text-muted-foreground">الرصيد</p>
                      <p className="font-bold text-sm mt-1 text-green-600 dark:text-green-400">{formatCurrency(proj.totals.balance)}</p>
                    </div>
                    <div className="text-center p-3 rounded-md bg-muted/30" data-testid={`stat-wages-${proj.projectId}`}>
                      <p className="text-xs text-muted-foreground">الأجور</p>
                      <p className="font-bold text-sm mt-1">{formatCurrency(proj.totals.totalWages)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            {filteredWorkers.length > 0 && (
              <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
                  <CardTitle className="text-base">ملخص العمالة المجمع</CardTitle>
                  <Badge variant="secondary">{filteredWorkers.length} عامل</Badge>
                </CardHeader>
                <CardContent>
                  <ReportTable
                    testId="table-multi-attendance"
                    headers={["اسم العامل", "المشروع", "النوع", "الأيام", "المستحق", "المدفوع", "الحوالات", "إجمالي المدفوع", "المتبقي"]}
                    rows={filteredWorkers.map((w: any) => [
                      w.workerName, w.projectName, w.workerType,
                      w.totalDays, formatCurrency(w.totalEarned),
                      formatCurrency(w.totalDirectPaid ?? w.totalPaid),
                      formatCurrency(w.totalTransfers ?? 0),
                      formatCurrency(w.totalPaid), formatCurrency(w.balance ?? 0),
                    ])}
                  />
                </CardContent>
              </Card>
            ...
            {filteredMaterials.length > 0 && (
              <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
                  <CardTitle className="text-base">ملخص المواد المجمع</CardTitle>
                  <Badge variant="secondary">{filteredMaterials.length} مادة</Badge>
                </CardHeader>
                <CardContent>
                  <ReportTable
                    testId="table-multi-materials"
                    headers={["اسم المادة", "المشروع", "الكمية", "الإجمالي", "المورد"]}
                    rows={filteredMaterials.map((m: any) => [
                      m.materialName, m.projectName, m.totalQuantity,
                      formatCurrency(m.totalAmount), m.supplierName || "-",
                    ])}
                  />
                </CardContent>
              </Card>
            ...
            {filteredInterTransfers.length > 0 && (
              <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
                  <CardTitle className="text-base">التحويلات بين المشاريع المحددة</CardTitle>
                  <Badge variant="secondary">{filteredInterTransfers.length}</Badge>
                </CardHeader>
                <CardContent>
                  <ReportTable
                    testId="table-multi-inter-transfers"
                    headers={["التاريخ", "من مشروع", "إلى مشروع", "المبلغ", "السبب"]}
                    rows={filteredInterTransfers.map((t: any) => [
            ...
            {filteredFundTransfers.length > 0 && (
              <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
                  <CardTitle className="text-base">التحويلات المالية المجمعة</CardTitle>
                  <Badge variant="secondary">{filteredFundTransfers.length}</Badge>
                </CardHeader>
                <CardContent>
                  <ReportTable
                    testId="table-multi-fund-transfers"
                    headers={["التاريخ", "المبلغ", "المرسل", "النوع", "المشروع"]}
                    rows={filteredFundTransfers.map((f: any) => [
                      safeFormatDate(f.date, "dd/MM/yyyy"),
                      formatCurrency(f.amount),
                      f.senderName || "-",
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">الملخص المالي الشامل المجمع</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-3 rounded-md bg-muted/30" data-testid="stat-combined-materials">
                    <p className="text-xs text-muted-foreground">المواد</p>
                    <p className="font-bold text-sm mt-1">{formatCurrency(multiReport.combinedTotals.totalMaterials)}</p>
                  </div>
                  <div className="text-center p-3 rounded-md bg-muted/30" data-testid="stat-combined-transport">
                    <p className="text-xs text-muted-foreground">النقل</p>
                    <p className="font-bold text-sm mt-1">{formatCurrency(multiReport.combinedTotals.totalTransport)}</p>
                  </div>
...
function ProjectComprehensiveTab({ onStatsReady }: { onStatsReady?: (stats: any[]) => void }) {
  const { selectedProjectId, selectedProjectName, isAllProjects } = useSelectedProjectContext();
  const { toast } = useToast();
  const [searchValue, setSearchValue] = useState("");
  const [dateRange, setDateRange] = useState<{ from?: Date; to?: Date }>({});
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  القوى العاملة ({report.workforce.totalWorkers} عامل، {report.workforce.activeWorkers} نشط)
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  الآبار ({report.wells.totalWells} بئر — متوسط الإنجاز {report.wells.avgCompletionPercentage.toFixed(0)}%)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  المعدات ({report.equipmentSummary.totalEquipment})
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                    ...
                    <tbody>
                      {report.equipmentSummary.items.map((eq, i) => (
                        <tr key={i} className="border-b hover:bg-muted/30">
                          ...
                          <td className="p-2">
                            <Badge variant={eq.status === 'active' ? 'default' : 'outline'}>{eq.status === 'active' ? 'نشط' : eq.status}</Badge>
                          </td>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4" data-testid="cash-custody-summary">
                <div className="text-center p-3 bg-green-50 dark:bg-green-950 rounded-lg">
                  <div className="text-xs text-muted-foreground">التحويلات الواردة</div>
                  <div className="text-lg font-bold text-green-600">{formatCurrency(report.cashCustody.totalFundTransfersIn)}</div>
                </div>
                <div className="text-center p-3 bg-blue-50 dark:bg-blue-950 rounded-lg">
                  <div className="text-xs text-muted-foreground">من مشاريع أخرى</div>
                  <div className="text-lg font-bold text-blue-600">{formatCurrency(report.cashCustody.totalProjectTransfersIn)}</div>
                </div>
                <div className="text-center p-3 bg-red-50 dark:bg-red-950 rounded-lg">
                  <div className="text-xs text-muted-foreground">لمشاريع أخرى</div>
                  <div className="text-lg font-bold text-red-600">{formatCurrency(report.cashCustody.totalProjectTransfersOut)}</div>
                </div>
                <div className="text-center p-3 bg-purple-50 dark:bg-purple-950 rounded-lg">
                  <div className="text-xs text-muted-foreground">صافي الرصيد</div>
                  <div className="text-lg font-bold text-purple-600">{formatCurrency(report.cashCustody.netBalance)}</div>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm" data-testid="table-comprehensive-transfers">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        ...
                        <th className="p-2 text-right">المرسل</th>
                        <th className="p-2 text-right">النوع</th>
