...
- **إصلاح Rate Limit الذي يحظر تسجيل الدخول من الموبايل (P0)**: `generalRateLimit` في `server/middleware/auth.ts` كان يطبَّق على كل المسارات بمفتاح IP فقط بحد 600/15min. الموبايل خلف CGNAT (Carrier-Grade NAT) يجعل عشرات المستخدمين يخرجون من نفس IP عام → الحد ينفجر بسرعة → رسالة "تم تجاوز الحد المسموح من الطلبات" تمنع تسجيل الدخول. الإصلاح (معيار Stripe/GitHub): (1) `keyGenerator` يستخدم `user_id` من JWT إن وُجد قبل الرجوع لـ IP، (2) استثناء `/api/auth/*` و `/api/webauthn/*` (لها authRateLimit مخصص، 10/15min بمفتاح IP+email)، (3) استثناء `/api/notifications/stream` (SSE long-lived)، (4) استثناء أصول Vite/static و sw.js، (5) رفع الحد العام من 600 إلى 2000.
...
- **تدوير keystore منفّذ (2026-04-21)**: تمّ توليد keystore جديد (RSA 4096، صالح حتى 2053) على السيرفر `/home/administrator/.axion-keystore/axion-release.keystore` مع نسخة احتياطية للقديم في `backups/`. `KEYSTORE_PASSWORD` و `KEYSTORE_KEY_PASSWORD` محدَّثتان في Replit Secrets. SHA-1 الجديد `7A:14:D0:2E:37:A2:70:4D:5C:59:DE:67:26:CF:0A:7D:B9:AE:CF:76` مُسجَّل في Firebase ومُقيِّد API keys في Google Cloud Console لـ package `com.axion.app`.
- **تدوير Firebase Service Account (2026-04-21)**: المفتاح النشط الحالي `private_key_id: d908ac5e...` (المفاتيح القديمة `e0465a1a` و `fe85d43a` حُذفت من Google Cloud IAM). مُحدَّث في `FIREBASE_SERVICE_ACCOUNT_KEY` Secret. تأكد التهيئة الناجحة في السجلات (`✅ [FCM] Firebase Admin تم التهيئة بنجاح`). `google-services.json` الجديد محدَّث محلياً ومرفوع للسيرفر في `/home/administrator/.axion-secrets/google-services.json`. أُضيف fallback في `deployment-engine.ts` لنسخ الملف من المسار الآمن أثناء النشر إن لم يكن في المستودع.
