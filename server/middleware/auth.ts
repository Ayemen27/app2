...
export const authenticate = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    // ✅ المسارات التي تبدأ بـ prefix معين تكون عامة
    const publicPrefixes = [
      '/api/sync/',           // جميع مسارات المزامنة
      '/api/backups/download/' // تحميل النسخ الاحتياطية
    ];
    ...
    const isAuthRoute = req.originalUrl.includes('/api/auth/login') || 
    ...
    if (isPublicPath || isPublicPrefix || isAuthRoute) {
      return next();
    }
  try {
