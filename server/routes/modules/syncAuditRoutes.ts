...
router.delete('/by-status/:status', async (req: Request, res: Response) => {
  try {
    const validStatuses = ['success', 'failed', 'duplicate', 'conflict', 'skipped'];
    const { status } = req.params;
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ success: false, message: `حالة غير صالحة: ${status}` });
    }
...
router.post('/purge-old', async (req: Request, res: Response) => {
  try {
    const { olderThanDays = 90 } = req.body;
    const days = Math.max(7, Math.min(365, Number(olderThanDays) || 90));
    const result = await SyncAuditService.purgeLogs(days);
    ...
    res.status(500).json({ success: false, message: 'خطأ في تنظيف السجلات القديمة' });
  }
});
