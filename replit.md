...
- **Authentication & Security:** JWT-based authentication with access/refresh tokens, WebAuthn/FIDO2 for biometric login, robust rate limiting on all auth endpoints (login, register, verify-email, validate-field, emergency login/create, resend-verification, forgot/reset-password), token hardening (httpOnly cookies), mass-assignment protection via field allowlists, API response hardening, SQL injection prevention, canonical auth identity, centralized RBAC, and strict CORS origin validation. Session security includes client context extraction, session binding, verification with device binding, refresh token rotation, and replay protection. Admin-only enforcement on sensitive operations: batch sync, inventory adjust/delete, settlement delete, worker type creation, worker transfer/attendance/misc-expense modification, wage backfill. All financial mutations use atomic DB transactions (withTransaction). Project data access filtered by accessibleProjectIds for non-admin users across all endpoints including fund-transfers, material-purchases-unified, and all-projects-expenses. Zod schema validation on all POST/PUT/PATCH endpoints across wells, inventory, settlements, materials, and financial routes.
