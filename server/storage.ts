...
export class DatabaseStorage implements IStorage {
  ...
  async createFundTransfer(transfer: InsertFundTransfer): Promise<FundTransfer> {
    try {
      ...
      const transferDateStr = transfer.transferDate ? String(transfer.transferDate) : new Date().toISOString();
      let transferDate: string;
      try {
        transferDate = new Date(transferDateStr).toISOString().split('T')[0];
      } catch (e) {
  ...
  async deleteFundTransfer(id: string): Promise<void> {
    ...
    if (transfer) {
      const transferDateStr = transfer.transferDate ? String(transfer.transferDate) : new Date().toISOString();
      const transferDate = new Date(transferDateStr).toISOString().split('T')[0];
