#!/bin/bash
# AXION AI Build Engine v6.5.0 (Nuclear-Resilience & Deep-Healing)
# Designed for SDK 35 & Java 21 compatibility

set -e
LOG_FILE="/tmp/axion_smart_build_$(date +%s).log"
APK_OUTPUT_DIR="/home/administrator/app2/output_apks"
ANDROID_ROOT="/home/administrator/app2/android"
mkdir -p "$APK_OUTPUT_DIR"

log() {
    local timestamp=$(date "+%Y-%m-%d %H:%M:%S")
    echo -e "\e[32m[$timestamp] >>> $1\e[0m" | tee -a "$LOG_FILE"
}

error_exit() {
    local timestamp=$(date "+%Y-%m-%d %H:%M:%S")
    echo -e "\e[31m[$timestamp] ❌ ERROR: $1\e[0m" | tee -a "$LOG_FILE"
    exit 1
}

repair_environment() {
    log "Initializing Smart Repair Engine..."
    
    # 1. Java 21 Enforcement (Prioritize OpenJDK 21)
    export JAVA_HOME="/usr/lib/jvm/java-21-openjdk-amd64"
    if [ ! -d "$JAVA_HOME" ]; then
        JAVA_HOME=$(readlink -f /usr/bin/java | sed "s:/bin/java::")
        log "Warning: Standard Java 21 path not found. Using detected: $JAVA_HOME"
    fi
    
    # 2. Path Sanitation (NUCLEAR OPTION)
    # We rebuild the PATH from scratch to ensure NO system gradle (4.4.1) can be found
    log "Rebuilding PATH for absolute isolation..."
    export PATH="$JAVA_HOME/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
    
    # 3. Gradle Wrapper Deep Repair
    log "Configuring Gradle 8.5 Wrapper..."
    local WRAPPER_PROPS="$ANDROID_ROOT/gradle/wrapper/gradle-wrapper.properties"
    local WRAPPER_JAR="$ANDROID_ROOT/gradle/wrapper/gradle-wrapper.jar"
    
    mkdir -p "$(dirname "$WRAPPER_PROPS")"
    cat <<EOF > "$WRAPPER_PROPS"
distributionBase=GRADLE_USER_HOME
distributionPath=wrapper/dists
zipStoreBase=GRADLE_USER_HOME
zipStorePath=wrapper/dists
distributionUrl=https\://services.gradle.org/distributions/gradle-8.5-all.zip
EOF

    log "Downloading fresh Gradle Wrapper JAR (v8.5)..."
    wget -qO "$WRAPPER_JAR" "https://raw.githubusercontent.com/gradle/gradle/v8.5.0/gradle/wrapper/gradle-wrapper.jar" || \
    curl -sLo "$WRAPPER_JAR" "https://raw.githubusercontent.com/gradle/gradle/v8.5.0/gradle/wrapper/gradle-wrapper.jar"
    
    chmod +x "$ANDROID_ROOT/gradlew"

    # 4. Global Environment Lockdown
    export TERM=dumb
    export GRADLE_OPTS="-Dorg.gradle.java.home=$JAVA_HOME -Dorg.gradle.daemon=false"
    unset GRADLE_HOME
    unset GRADLE_USER_HOME
    
    log "Environment ready. Java: $(java -version 2>&1 | head -n 1)"
}

repair_project_files() {
    log "Applying Project Fixes for SDK 35..."
    local APP_GRADLE="$ANDROID_ROOT/app/build.gradle"
    local PROJ_GRADLE="$ANDROID_ROOT/build.gradle"
    local MANIFEST="$ANDROID_ROOT/app/src/main/AndroidManifest.xml"

    # Fix Project Level (AGP 8.2.2)
    if [ -f "$PROJ_GRADLE" ]; then
        sed -i "s/classpath ['\"]com.android.tools.build:gradle:[0-9.]*['\"]/classpath 'com.android.tools.build:gradle:8.2.2'/g" "$PROJ_GRADLE"
    fi

    # Fix App Level (SDK 35)
    if [ -f "$APP_GRADLE" ]; then
        sed -i 's/minSdk [0-9]*/minSdk 24/g' "$APP_GRADLE" # Increased for better compatibility with 8.x
        sed -i 's/compileSdk [0-9]*/compileSdk 35/g' "$APP_GRADLE"
        sed -i 's/targetSdk [0-9]*/targetSdk 34/g' "$APP_GRADLE"
        sed -i 's/namespace "/namespace = "/g' "$APP_GRADLE"
        
        # Ensure MultiDex
        grep -q "multiDexEnabled true" "$APP_GRADLE" || sed -i '/defaultConfig {/a \        multiDexEnabled true' "$APP_GRADLE"
    fi
}

log "Starting AXION Smart Build Engine v6.5.0"
repair_environment
repair_project_files

log "Initiating Build Process..."
cd "$ANDROID_ROOT"

# CRITICAL: We call Java to run the Gradle Wrapper directly to bypass any shell 'gradle' aliases
if ! "$JAVA_HOME/bin/java" -jar "gradle/wrapper/gradle-wrapper.jar" assembleDebug --no-daemon; then
    log "Build failed. Attempting clean retry..."
    "$JAVA_HOME/bin/java" -jar "gradle/wrapper/gradle-wrapper.jar" clean
    "$JAVA_HOME/bin/java" -jar "gradle/wrapper/gradle-wrapper.jar" assembleDebug -x lint --no-daemon || error_exit "Build failed."
fi

APK_FILE=$(find app/build/outputs/apk/debug -name "*.apk" | head -n 1)
if [ -f "$APK_FILE" ]; then
    FINAL_NAME="AXION_FINAL_$(date +%s).apk"
    cp "$APK_FILE" "$APK_OUTPUT_DIR/$FINAL_NAME"
    log "✅ SUCCESS: APK at $APK_OUTPUT_DIR/$FINAL_NAME"
else
    error_exit "APK generation failed."
fi
