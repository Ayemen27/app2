...
interface PDFGenerationOptions {
  /** الوضع القديم: HTML واحد يُحوَّل إلى صورة ويُقسّم على صفحات (الترويسة لا تتكرر). */
  html?: string;
  /** الوضع الجديد (مُستحسَن): HTML الترويسة (يتكرر في رأس كل صفحة). */
  headerHtml?: string;
  /** الوضع الجديد (مُستحسَن): HTML المحتوى الرئيسي (يُقسَّم على صفحات تحت الترويسة). */
  contentHtml?: string;
  /** الوضع الجديد (مُستحسَن): HTML التذييل (يتكرر في أسفل كل صفحة). */
  footerHtml?: string;
  filename: string;
  format?: 'A4' | 'Letter';
...
/** يلتقط عنصر HTML داخلي بصورة canvas مؤقتاً (يُضاف خارج الشاشة ثم يُحذف). */
async function _captureHtmlAsCanvas(
  html: string,
  ...
  // ننتظر تحميل أي صور (شعار/إلخ) قبل التقاط الصورة
...
export async function generatePDF(options: PDFGenerationOptions): Promise<boolean> {
  try {
    ...
    const containerWidth = isLandscape ? 1122 : 794;
    const imgWidthMm = isLandscape ? 297 : 210;
    ...
    // ─────────────────────────────────────────────────────────────
    // الوضع الجديد: header/content/footer منفصلة → تكرار الترويسة في كل صفحة
    // ─────────────────────────────────────────────────────────────
    if (options.headerHtml && options.contentHtml) {
      const headerCanvas = await _captureHtmlAsCanvas(options.headerHtml, containerWidth);
      const contentCanvas = await _captureHtmlAsCanvas(options.contentHtml, containerWidth);
      const footerCanvas = options.footerHtml
        ? await _captureHtmlAsCanvas(options.footerHtml, containerWidth)
        : null;

      const headerImg = headerCanvas.toDataURL('image/jpeg', 0.92);
      const contentImg = contentCanvas.toDataURL('image/jpeg', 0.92);
      const footerImg = footerCanvas ? footerCanvas.toDataURL('image/jpeg', 0.92) : null;

      // تحويل الأبعاد إلى ملليمترات
      const headerHmm = (headerCanvas.height * imgWidthMm) / headerCanvas.width;
      const footerHmm = footerCanvas ? (footerCanvas.height * imgWidthMm) / footerCanvas.width : 0;
      const contentTotalHmm = (contentCanvas.height * imgWidthMm) / contentCanvas.width;

      // المساحة المتاحة لمحتوى كل صفحة (بعد طرح الترويسة والتذييل)
      const usableHmm = a4HeightMm - headerHmm - footerHmm;
      if (usableHmm <= 10) {
        // الترويسة/التذييل أكبر من اللازم؛ ارجع للوضع البسيط
        pdf.addImage(headerImg, 'JPEG', 0, 0, imgWidthMm, headerHmm);
        if (footerImg) pdf.addImage(footerImg, 'JPEG', 0, a4HeightMm - footerHmm, imgWidthMm, footerHmm);
      } else {
        let drawnMm = 0;
        let firstPage = true;
        while (drawnMm < contentTotalHmm - 0.5) {
          if (!firstPage) pdf.addPage();
          firstPage = false;

          // 1) الترويسة في رأس الصفحة
          pdf.addImage(headerImg, 'JPEG', 0, 0, imgWidthMm, headerHmm);

          // 2) شريحة من المحتوى تحتها — نُموضع الصورة الكاملة بـ y سالب لكشف الجزء المطلوب
          const remainingMm = contentTotalHmm - drawnMm;
          const sliceHmm = Math.min(usableHmm, remainingMm);
          const yOffset = headerHmm - drawnMm; // إزاحة الصورة لأعلى لإظهار الجزء غير المرسوم
          // قَصّ منطقة المحتوى عبر مستطيل قصّ
          pdf.saveGraphicsState();
          // jsPDF clipping يعمل عبر rectangle path ثم clip()
          pdf.rect(0, headerHmm, imgWidthMm, sliceHmm).clip().discardPath();
          pdf.addImage(contentImg, 'JPEG', 0, yOffset, imgWidthMm, contentTotalHmm);
          pdf.restoreGraphicsState();

          drawnMm += sliceHmm;

          // 3) التذييل في أسفل الصفحة
          if (footerImg) {
            pdf.addImage(footerImg, 'JPEG', 0, a4HeightMm - footerHmm, imgWidthMm, footerHmm);
          }
        }
      }
    } else if (options.html) {
      // ─────────────────────────────────────────────────────────────
      // الوضع القديم: HTML واحد (للحفاظ على التوافق مع المستدعين القُدامى)
      // ─────────────────────────────────────────────────────────────
      const canvas = await _captureHtmlAsCanvas(options.html, containerWidth);
      const imgHeight = (canvas.height * imgWidthMm) / canvas.width;
      const imgData = canvas.toDataURL('image/jpeg', 0.92);

      if (imgHeight <= a4HeightMm) {
        pdf.addImage(imgData, 'JPEG', 0, 0, imgWidthMm, imgHeight);
      } else {
        let heightLeft = imgHeight;
        let position = 0;
        pdf.addImage(imgData, 'JPEG', 0, position, imgWidthMm, imgHeight);
      ...
      throw new Error('generatePDF: must provide either html OR headerHtml+contentHtml');
    }
    ...
    console.error('[pdfGenerator] generatePDF failed:', error);
    return false;
  }
}
...
export async function generateTablePDF(options: TablePDFOptions): Promise<boolean> {
  ...
  // 🎨 لون الحدود: مشتق من اللون الثانوي للإعدادات (موحَّد مع لون رأس الجدول)
  const borderColor = _b.secondaryColor || '#334155';
  ...
  const wrapperOpen = `<div dir="rtl" lang="ar" style="font-family:'Cairo','Segoe UI',Tahoma,sans-serif;background:#fff;padding:0;margin:0;width:${containerWidth}px;">`;
  const wrapperClose = `</div>`;

  // 🏛️ ترويسة منفصلة (تتكرر في رأس كل صفحة)
  const headerHtml = `${wrapperOpen}${buildLetterheadHeader(options.reportTitle)}${wrapperClose}`;

  // 📄 المحتوى الرئيسي (subtitle + info + الجدول + التواقيع)
  const contentHtml = `${wrapperOpen}
    ...
    <div style="display:flex;justify-content:space-around;padding:20px 40px;margin-top:10px;">
      <div style="text-align:center;font-size:10px;"><b>توقيع المهندس</b><br/><span style="font-size:11px;font-weight:700;color:${hdrColor};">${engineerName || '.................................'}</span></div>
  ...
  // 🦶 تذييل منفصل (يتكرر في أسفل كل صفحة)
  const footerHtml = `${wrapperOpen}
