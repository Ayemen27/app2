...
export class ExpenseLedgerService {
  ...
  static async getProjectFinancialSummary(projectId: string, date?: string, dateFrom?: string, dateTo?: string): Promise<any> {
    try {
      ...
      const dateFilterMp = cleanDate ? sql`AND (CASE WHEN purchase_date IS NULL OR purchase_date = '' OR purchase_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE purchase_date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN purchase_date IS NULL OR purchase_date = '' OR purchase_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE purchase_date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterWa = cleanDate ? sql`AND (CASE WHEN attendance_date IS NULL OR attendance_date = '' OR attendance_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE attendance_date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN attendance_date IS NULL OR attendance_date = '' OR attendance_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE attendance_date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterTe = cleanDate ? sql`AND (CASE WHEN date IS NULL OR date = '' OR date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN date IS NULL OR date = '' OR date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterWt = cleanDate ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date = '' OR transfer_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date = '' OR transfer_date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterMwe = cleanDate ? sql`AND (CASE WHEN date IS NULL OR date = '' OR date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN date IS NULL OR date = '' OR date !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterFt = cleanDate ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date::text = '' OR transfer_date::text !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date::text = '' OR transfer_date::text !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
      const dateFilterPft = cleanDate ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date::text = '' OR transfer_date::text !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) = ${cleanDate}::date` : (cleanDateFrom && cleanDateTo ? sql`AND (CASE WHEN transfer_date IS NULL OR transfer_date::text = '' OR transfer_date::text !~ '^\\d{4}-\\d{2}-\\d{2}' THEN NULL ELSE transfer_date::date END) BETWEEN ${cleanDateFrom}::date AND ${cleanDateTo}::date` : sql`AND 1=1`);
