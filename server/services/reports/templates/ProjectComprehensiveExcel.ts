...
export async function generateProjectComprehensiveExcel(data: ProjectComprehensiveReportData): Promise<Buffer> {
  ...
  if (data.workforce.topWorkers.length > 0) {
    ...
    row = xlTableHeader(ws, row, ['#', 'الاسم', 'النوع', 'الأيام', 'المستحق', 'المدفوع', 'الحوالات', 'المتبقي', '']);
    let xlTopDays = 0, xlTopEarned = 0, xlTopPaid = 0, xlTopTransfers = 0, xlTopBalance = 0;
    data.workforce.topWorkers.forEach((w, i) => {
      ...
      row = xlDataRow(ws, row, [i + 1, w.name, w.type, formatNum(w.totalDays), formatNum(w.totalEarned), formatNum(w.totalPaid), formatNum(w.totalTransfers || 0), formatNum(w.balance), ''], i % 2 === 1);
    });
    row = xlGrandTotalRow(ws, row, ['', 'الإجمالي', '', formatNum(xlTopDays), formatNum(xlTopEarned), formatNum(xlTopPaid), formatNum(xlTopTransfers), formatNum(xlTopBalance), '']);
    row++;
  }
