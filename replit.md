...
- **Deployment Engine Security & Stability Hardening (March 2026):** 8 critical fixes applied:
  ...
  17. **TypeScript Error Fixes (March 2026):** Fixed 4 TS compile errors: `transfer_method` → `transferMethod` in financialRoutes, `id` → `transferId` in workerRoutes PATCH handler, `safePaidAmount` scope lifted from inner `if` block to handler level in attendance PATCH.
