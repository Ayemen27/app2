...
- **Authentication & Security:** JWT-based authentication with access/refresh tokens (strict expiration enforcement — no `ignoreExpiration`), WebAuthn/FIDO2 for biometric login, rate limiting, and robust logging sanitization. Tokens accepted only from Authorization header and cookies (no query strings, no custom headers). Admin tokens use same lifetime as regular users (no privilege inflation). Offline login uses PBKDF2 hashing, generating offline credentials from plaintext passwords at online login. Canonical auth identity via `AuthUser` interface in `server/internal/auth-user.ts` with helpers: `getAuthUser()`, `requireAuthUser()`, `getAuthUserId()`, `isAdmin()`. Session revocation enforced in `authenticate` middleware. Centralized RBAC via `server/middleware/authz.ts` (`requireRoles()`, `requireAdmin()`). `requireRole('admin')` in `auth.ts` automatically includes `super_admin` — no role-check drift possible. All route files use typed auth access — no `(req as any).user` or email/admin fallbacks. Monitoring endpoints (`/api/monitoring/dashboard-stats`, `/api/monitoring/devices`, `/api/monitoring/alert`) are admin-only via router-level middleware. Health endpoints (`/api/health/full`, `/api/health/history`, `/api/health/circuits`, `/api/db/info`) require authentication. Autocomplete admin routes require auth + admin. Notification stats (`/stats`, `/monitoring/stats`) require admin. Announcement creation requires admin (enforced in handler even for `POST /:type` parameterized route). Sync routes (`/api/sync/full-backup`, `/api/sync/instant-sync`, `/api/sync/verify-sync`, `/api/sync/stats`, `/api/sync/tables`) are admin-only. Batch sync (`/api/sync/batch`) enforces per-operation project-level authorization via `ProjectAccessService` with server-side row ownership verification for UPDATE/DELETE operations. Auth regression test suite: 37 tests in `server/tests/auth-regression.test.ts`. CORS origin validation uses strict URL parsing via `new URL()` with exact hostname matching and controlled subdomain policy — no `includes()`/`startsWith()` patterns. CORS regression test suite: 27 tests in `server/tests/cors-regression.test.ts`. Preflight (OPTIONS) handler validates origins through same `isOriginAllowed()` function.
- **WhatsApp Reconnect Safety:** Conflict (440) reconnect uses dual protection: per-attempt counter (max 3 consecutive) and sliding-window tracker (max 5 conflicts per 5 minutes) to permanently halt reconnect loops caused by competing sessions.
...
- **AI Agent:** Processes messages via `/api/ai/chat` (rate-limited: 20 req/60s per user), utilizing `DatabaseActions.ts` for CRUD, introspection, and analytics on whitelisted tables. Sensitive tables (`users`, `auth_user_sessions`, `password_reset_tokens`, `emergency_users`, `webauthn_credentials`, `whatsapp_user_links`, `whatsapp_allowed_numbers`, `refresh_tokens`) are blocked from generic CRUD. Raw SQL execution restricted to SELECT-only with dangerous PostgreSQL functions blocked (`pg_read_file`, `pg_ls_dir`, etc.) and `SET TRANSACTION READ ONLY` enforced. User input sanitized to strip `[ACTION:...]` and `[PROPOSE:...]` patterns before LLM processing. CORS restricted to known origins only. It features a model fallback system (HuggingFace Llama 3.1 8B → Gemini 2.0 Flash → OpenAI GPT-4o) and intent detection. Write operations require user confirmation.
...
- **Reporting System:** Generates professional daily, worker statement, period-final, and attendance reports, exportable to Excel and PDF with RTL support and a unified design system (`axion-export.ts`). Includes multi-project combined reports with detailed breakdowns and inter-project transfer tracking. Attendance report (`ReportGenerator.generateAttendanceReport`) queries all workers' attendance for a project within a date range using parameterized SQL, produces JSON or Excel (with worker-level summaries and project-level totals), and supports `formatAsText` for WhatsApp/chat rendering.
