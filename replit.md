...
- **Fail-Closed Startup:** Server now exits with `process.exit(1)` if startup migrations fail. Previously fail-open (server continued without critical DB functions). File: `server/index.ts`.
