 ...
 * ملاحظة: مسار /api/sync/full-backup عام ولا يتطلب توكن
 */
export async function performInitialDataPull(): Promise<boolean> {
  // فحص الإنترنت فقط - المسار عام ولا يحتاج توكن
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    console.warn('📡 [Sync] لا يمكن السحب الأولي بدون إنترنت');
    return false;
  }
  
  // تسجيل حالة التوكن للتشخيص (لكن لا نمنع المزامنة)
  const accessToken = localStorage.getItem('accessToken');
  if (!accessToken) {
    console.log('ℹ️ [Sync] المزامنة الأولية بدون توكن (المسار عام)');
  }
