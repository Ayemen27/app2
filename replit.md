...
## حالة المشروع الحالية (24 يناير 2026):
...
## نظام قواعد البيانات الموحد (24 يناير 2026)

### ترتيب أولوية الاتصال:
| الأولوية | المتغير | الوصف |
|----------|---------|-------|
| 1 | `DATABASE_URL_CENTRAL` | القاعدة المركزية الرئيسية (PostgreSQL خارجي) |
| 2 | `DATABASE_URL_SUPABASE` | قاعدة Supabase/External |
| 3 | `DATABASE_URL_RAILWAY` | قاعدة Railway |
| 4 | `DATABASE_URL` | القاعدة الافتراضية (Replit Helium) |

### القاعدة المركزية الحالية:
- **الخادم**: `93.127.142.144:5432`
- **القاعدة**: `newdb`
- **المستخدم**: `newuser`
- **الإصدار**: PostgreSQL 16.11

### إحصائيات القاعدة المركزية:
- **عدد الجداول**: 68 جدول
- **إجمالي السجلات**: ~3,272 سجل
- **الجداول النشطة**: autocomplete_data (764), worker_attendance (596), transportation_expenses (300), notifications (287)

### نظام الطوارئ:
- يتفعل تلقائياً عند فشل الاتصال بالقاعدة المركزية
- يستخدم SQLite محلي (`local.db`)
- يستعيد البيانات من آخر نسخة احتياطية
- يعاود الاتصال بالمركزية عند توفرها
